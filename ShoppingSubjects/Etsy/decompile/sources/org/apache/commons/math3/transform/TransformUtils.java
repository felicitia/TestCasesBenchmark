...
public class TransformUtils {
    private static final int[] POWERS_OF_TWO = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 67108864, 134217728, ErrorDialogData.BINDER_CRASH, ErrorDialogData.DYNAMITE_CRASH, ErrorDialogData.SUPPRESSED};
    ...
    public static double[][] createRealImaginaryArray(Complex[] complexArr) {
        double[][] dArr = (double[][]) Array.newInstance(double.class, new int[]{2, complexArr.length});
        double[] dArr2 = dArr[0];
        double[] dArr3 = dArr[1];
        for (int i = 0; i < complexArr.length; i++) {
