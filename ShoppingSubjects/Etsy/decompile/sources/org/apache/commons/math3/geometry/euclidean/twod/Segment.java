...
public class Segment {
    ...
    public double distance(Vector2D vector2D) {
        ...
        if (x2 < 0.0d || x2 > 1.0d) {
            return FastMath.min(getStart().distance(vector2D), getEnd().distance(vector2D));
        }
        return new Vector2D(this.start.getX() + (x * x2), this.start.getY() + (x2 * y)).distance(vector2D);
    }
}
