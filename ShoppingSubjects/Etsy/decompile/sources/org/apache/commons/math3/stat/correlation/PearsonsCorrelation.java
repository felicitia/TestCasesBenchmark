...
public class PearsonsCorrelation {
    ...
    public RealMatrix getCorrelationStandardErrors() {
        ...
        double[][] dArr = (double[][]) Array.newInstance(double.class, new int[]{columnDimension, columnDimension});
        ...
        for (int i = 0; i < columnDimension; i++) {
            for (int i2 = 0; i2 < columnDimension; i2++) {
                if (i == i2) {
                    ...
                    double entry = this.correlationMatrix.getEntry(i, i2);
                    dArr[i][i2] = 2.0d * tDistribution.cumulativeProbability(-FastMath.abs(entry * FastMath.sqrt(((double) (this.nObs - 2)) / (1.0d - (entry * entry)))));
                }
            }
        }
        return new BlockRealMatrix(dArr);
