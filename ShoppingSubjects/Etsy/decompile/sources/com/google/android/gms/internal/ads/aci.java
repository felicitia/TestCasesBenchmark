package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.support.v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class aci extends ach {
    private static final Object s = new Object();
    private static final String t = "aci";
    @VisibleForTesting
    private static boolean u = false;
    private static long v;
    protected boolean r = false;
    private String w;
    private boolean x = false;
    private boolean y = false;
    @VisibleForTesting
    private zzdi z;

    protected aci(Context context, String str, boolean z2) {
        super(context);
        this.w = str;
        this.r = z2;
    }

    private static adf a(acy acy, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzcw {
        Method a = acy.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new adf((String) a.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    protected static synchronized void a(Context context, boolean z2) {
        synchronized (aci.class) {
            if (!u) {
                v = System.currentTimeMillis() / 1000;
                a = b(context, z2);
                u = true;
            }
        }
    }

    private static void a(List<Callable<Void>> list) {
        if (a != null) {
            ExecutorService c = a.c();
            if (c != null && !list.isEmpty()) {
                try {
                    c.invokeAll(list, ((Long) ajh.f().a(akl.bD)).longValue(), TimeUnit.MILLISECONDS);
                } catch (InterruptedException e) {
                    Log.d(t, String.format("class methods got exception: %s", new Object[]{adg.a((Throwable) e)}));
                }
            }
        }
    }

    private static acy b(Context context, boolean z2) {
        if (a == null) {
            synchronized (s) {
                if (a == null) {
                    acy a = acy.a(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z2);
                    if (a.b()) {
                        if (((Boolean) ajh.f().a(akl.bJ)).booleanValue()) {
                            a.a("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        a.a("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        a.a("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        a.a("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        a.a("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        a.a("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        a.a("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        a.a("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        a.a("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        a.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        a.a("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        a.a("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        a.a("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        a.a("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        a.a("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        a.a("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        a.a("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        a.a("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        a.a("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        a.a("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        a.a("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    a = a;
                }
            }
        }
        return a;
    }

    /* access modifiers changed from: protected */
    public final long a(StackTraceElement[] stackTraceElementArr) throws zzcw {
        Method a = a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (a == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new acw((String) a.invoke(null, new Object[]{stackTraceElementArr})).a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    /* access modifiers changed from: protected */
    public final vy a(Context context, View view, Activity activity) {
        List list;
        vy vyVar = new vy();
        if (!TextUtils.isEmpty(this.w)) {
            vyVar.b = this.w;
        }
        acy b = b(context, this.r);
        if (!b.b()) {
            vyVar.k = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            list = Arrays.asList(new Callable[]{new ads(b, vyVar)});
        } else {
            try {
                adf a = a(b, this.b, this.q);
                vyVar.g = a.a;
                vyVar.h = a.b;
                vyVar.i = a.c;
                if (this.p) {
                    vyVar.u = a.d;
                    vyVar.v = a.e;
                }
            } catch (zzcw unused) {
            }
            wu wuVar = new wu();
            if (this.d > 0 && adg.a(this.q)) {
                wuVar.m = Long.valueOf(adg.a(this.k, this.q));
                wuVar.n = Long.valueOf(adg.a((double) (this.n - this.l), this.q));
                wuVar.o = Long.valueOf(adg.a((double) (this.o - this.m), this.q));
                wuVar.r = Long.valueOf(adg.a((double) this.l, this.q));
                wuVar.s = Long.valueOf(adg.a((double) this.m, this.q));
                if (this.p && this.b != null) {
                    long a2 = adg.a((double) (((this.l - this.n) + this.b.getRawX()) - this.b.getX()), this.q);
                    if (a2 != 0) {
                        wuVar.p = Long.valueOf(a2);
                    }
                    long a3 = adg.a((double) (((this.m - this.o) + this.b.getRawY()) - this.b.getY()), this.q);
                    if (a3 != 0) {
                        wuVar.q = Long.valueOf(a3);
                    }
                }
            }
            try {
                adf b2 = b(this.b);
                wuVar.a = b2.a;
                wuVar.b = b2.b;
                wuVar.h = b2.c;
                if (this.p) {
                    wuVar.c = b2.e;
                    wuVar.e = b2.d;
                    wuVar.g = Integer.valueOf(b2.f.longValue() != 0 ? 1 : 0);
                    if (this.e > 0) {
                        wuVar.d = adg.a(this.q) ? Long.valueOf(Math.round(((double) this.j) / ((double) this.e))) : null;
                        wuVar.f = Long.valueOf(Math.round(((double) this.i) / ((double) this.e)));
                    }
                    wuVar.j = b2.i;
                    wuVar.i = b2.j;
                    wuVar.k = Integer.valueOf(b2.k.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw unused2) {
            }
            if (this.h > 0) {
                wuVar.l = Long.valueOf(this.h);
            }
            vyVar.K = wuVar;
            if (this.d > 0) {
                vyVar.y = Long.valueOf(this.d);
            }
            if (this.e > 0) {
                vyVar.x = Long.valueOf(this.e);
            }
            if (this.f > 0) {
                vyVar.w = Long.valueOf(this.f);
            }
            if (this.g > 0) {
                vyVar.z = Long.valueOf(this.g);
            }
            try {
                int size = this.c.size() - 1;
                if (size > 0) {
                    vyVar.L = new wu[size];
                    for (int i = 0; i < size; i++) {
                        adf a4 = a(a, (MotionEvent) this.c.get(i), this.q);
                        wu wuVar2 = new wu();
                        wuVar2.a = a4.a;
                        wuVar2.b = a4.b;
                        vyVar.L[i] = wuVar2;
                    }
                }
            } catch (zzcw unused3) {
                vyVar.L = null;
            }
            list = new ArrayList();
            if (b.c() != null) {
                int n = b.n();
                list.add(new ads(b, vyVar));
                acy acy = b;
                vy vyVar2 = vyVar;
                adv adv = new adv(acy, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", vyVar2, n, 1);
                list.add(adv);
                adq adq = new adq(acy, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", vyVar2, v, n, 25);
                list.add(adq);
                int i2 = n;
                adp adp = new adp(acy, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", vyVar2, i2, 44);
                list.add(adp);
                adu adu = new adu(acy, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", vyVar2, i2, 12);
                list.add(adu);
                adw adw = new adw(acy, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", vyVar2, i2, 3);
                list.add(adw);
                adt adt = new adt(acy, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", vyVar2, i2, 22);
                list.add(adt);
                ado ado = new ado(acy, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", vyVar2, i2, 5);
                list.add(ado);
                aee aee = new aee(acy, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", vyVar2, i2, 48);
                list.add(aee);
                adl adl = new adl(acy, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", vyVar2, i2, 49);
                list.add(adl);
                aeb aeb = new aeb(acy, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", vyVar2, i2, 51);
                list.add(aeb);
                aea aea = new aea(acy, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", vyVar2, i2, 45, new Throwable().getStackTrace());
                list.add(aea);
                aef aef = new aef(acy, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", vyVar2, i2, 57, view);
                list.add(aef);
                ady ady = new ady(acy, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", vyVar2, i2, 61);
                list.add(ady);
                if (((Boolean) ajh.f().a(akl.bE)).booleanValue()) {
                    adk adk = new adk(b, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", vyVar, n, 62, view, activity);
                    list.add(adk);
                }
                if (((Boolean) ajh.f().a(akl.bG)).booleanValue()) {
                    aec aec = new aec(b, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", vyVar, n, 53, this.z);
                    list.add(aec);
                }
            }
        }
        a(list);
        return vyVar;
    }

    /* access modifiers changed from: protected */
    public final vy a(Context context, ta taVar) {
        vy vyVar = new vy();
        if (!TextUtils.isEmpty(this.w)) {
            vyVar.b = this.w;
        }
        acy b = b(context, this.r);
        if (b.c() != null) {
            a(a(b, vyVar, (ta) null));
        }
        return vyVar;
    }

    /* access modifiers changed from: protected */
    public List<Callable<Void>> a(acy acy, vy vyVar, ta taVar) {
        int n = acy.n();
        ArrayList arrayList = new ArrayList();
        if (!acy.b()) {
            vyVar.k = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        acy acy2 = acy;
        vy vyVar2 = vyVar;
        adn adn = new adn(acy2, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", vyVar2, n, 27, taVar);
        arrayList.add(adn);
        adq adq = new adq(acy2, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", vyVar2, v, n, 25);
        arrayList.add(adq);
        int i = n;
        adv adv = new adv(acy2, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", vyVar2, i, 1);
        arrayList.add(adv);
        adx adx = new adx(acy2, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", vyVar2, i, 31);
        arrayList.add(adx);
        aed aed = new aed(acy2, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", vyVar2, i, 33);
        arrayList.add(aed);
        adm adm = new adm(acy2, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", vyVar2, i, 29);
        arrayList.add(adm);
        ado ado = new ado(acy2, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", vyVar2, i, 5);
        arrayList.add(ado);
        adu adu = new adu(acy2, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", vyVar2, i, 12);
        arrayList.add(adu);
        adw adw = new adw(acy2, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", vyVar2, i, 3);
        arrayList.add(adw);
        adp adp = new adp(acy2, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", vyVar2, i, 44);
        arrayList.add(adp);
        adt adt = new adt(acy2, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", vyVar2, i, 22);
        arrayList.add(adt);
        aee aee = new aee(acy2, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", vyVar2, i, 48);
        arrayList.add(aee);
        adl adl = new adl(acy2, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", vyVar2, i, 49);
        arrayList.add(adl);
        aeb aeb = new aeb(acy2, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", vyVar2, i, 51);
        arrayList.add(aeb);
        ady ady = new ady(acy2, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", vyVar2, i, 61);
        arrayList.add(ady);
        return arrayList;
    }

    public final void a(View view) {
        if (((Boolean) ajh.f().a(akl.bG)).booleanValue()) {
            if (this.z == null) {
                this.z = new zzdi(a, view);
            } else {
                this.z.zzd(view);
            }
        }
    }

    /* access modifiers changed from: protected */
    public final adf b(MotionEvent motionEvent) throws zzcw {
        Method a = a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (a == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new adf((String) a.invoke(null, new Object[]{motionEvent, this.q}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }
}
