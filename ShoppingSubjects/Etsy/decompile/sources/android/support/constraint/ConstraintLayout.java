...
public class ConstraintLayout extends ViewGroup {
    ...
    private void internalMeasureDimensions(int r27, int r28) {
        ...
        L_0x029e:
            r4.measure(r12, r14)
            android.support.constraint.solver.Metrics r12 = r0.mMetrics
    ...
    public void onMeasure(int i, int i2) {
        ...
        if (size3 > 0) {
            boolean z4 = this.mLayoutWidget.getHorizontalDimensionBehaviour() == DimensionBehaviour.WRAP_CONTENT;
            boolean z5 = this.mLayoutWidget.getVerticalDimensionBehaviour() == DimensionBehaviour.WRAP_CONTENT;
            ...
            int i24 = 0;
            while (i23 < size3) {
                ConstraintWidget constraintWidget = (ConstraintWidget) this.mVariableDimensionsWidgets.get(i23);
                    ...
                    if (layoutParams.isHelper || layoutParams.isGuideline) {
                        ...
                        if (view.getVisibility() != 8 && (!z3 || !constraintWidget.getResolutionWidth().isResolved() || !constraintWidget.getResolutionHeight().isResolved())) {
                            if (layoutParams.width != -2 || !layoutParams.horizontalDimensionFixed) {
                                i10 = MeasureSpec.makeMeasureSpec(constraintWidget.getWidth(), ErrorDialogData.SUPPRESSED);
                            } else {
                                i10 = getChildMeasureSpec(i15, paddingRight, layoutParams.width);
                            }
                            if (layoutParams.height != -2 || !layoutParams.verticalDimensionFixed) {
                                i11 = MeasureSpec.makeMeasureSpec(constraintWidget.getHeight(), ErrorDialogData.SUPPRESSED);
                            } else {
                                i11 = getChildMeasureSpec(i16, paddingBottom, layoutParams.height);
                            }
                            ...
                            if (measuredWidth != constraintWidget.getWidth()) {
                                ...
                                if (z4 && constraintWidget.getRight() > i22) {
                                    i22 = Math.max(i22, constraintWidget.getRight() + constraintWidget.getAnchor(Type.RIGHT).getMargin());
                                }
                                z6 = true;
                            }
                            if (measuredHeight != constraintWidget.getHeight()) {
                                ...
                                if (z5) {
                                    i14 = max2;
                                    if (constraintWidget.getBottom() > i14) {
                                        i13 = Math.max(i14, constraintWidget.getBottom() + constraintWidget.getAnchor(Type.BOTTOM).getMargin());
                                        i12 = i13;
                                        z6 = true;
                                    }
                                } else {
                            ...
                            if (VERSION.SDK_INT >= 11) {
                                i24 = combineMeasuredStates(i24, view.getMeasuredState());
                            } else {
                                int i25 = i24;
                            }
                            max2 = i12;
            ...
            i4 = i24;
            if (z6) {
                this.mLayoutWidget.setWidth(i26);
                this.mLayoutWidget.setHeight(i27);
                if (z3) {
                    this.mLayoutWidget.solveGraph();
                }
                solveLinearSystem("2nd pass");
                if (this.mLayoutWidget.getWidth() < i22) {
                    this.mLayoutWidget.setWidth(i22);
                    z = true;
                } else {
                    z = false;
                }
                if (this.mLayoutWidget.getHeight() < i29) {
                    this.mLayoutWidget.setHeight(i29);
                    z = true;
                }
                if (z) {
            ...
            for (int i31 = 0; i31 < i30; i31++) {
                ConstraintWidget constraintWidget2 = (ConstraintWidget) this.mVariableDimensionsWidgets.get(i31);
                ...
                if (view2 != null && (view2.getMeasuredWidth() != constraintWidget2.getWidth() || view2.getMeasuredHeight() != constraintWidget2.getHeight())) {
                    view2.measure(MeasureSpec.makeMeasureSpec(constraintWidget2.getWidth(), ErrorDialogData.SUPPRESSED), MeasureSpec.makeMeasureSpec(constraintWidget2.getHeight(), ErrorDialogData.SUPPRESSED));
                    if (this.mMetrics != null) {
        ...
        int width2 = this.mLayoutWidget.getWidth() + i3;
        int height2 = this.mLayoutWidget.getHeight() + paddingBottom;
        if (VERSION.SDK_INT >= 11) {
            int resolveSizeAndState = resolveSizeAndState(height2, i2, i4 << 16) & ViewCompat.MEASURED_SIZE_MASK;
            int min = Math.min(this.mMaxWidth, resolveSizeAndState(width2, i, i4) & ViewCompat.MEASURED_SIZE_MASK);
