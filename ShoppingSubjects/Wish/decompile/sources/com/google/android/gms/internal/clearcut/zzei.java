...
class zzei<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    private final int zza(K k) {
        ...
        if (size >= 0) {
            int compareTo = k.compareTo((Comparable) ((zzep) this.zzom.get(size)).getKey());
            if (compareTo > 0) {
                return -(size + 2);
            }
            if (compareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int compareTo2 = k.compareTo((Comparable) ((zzep) this.zzom.get(i2)).getKey());
            if (compareTo2 < 0) {
                size = i2 - 1;
    ...
    public boolean containsKey(Object obj) {
        ...
        return zza((K) comparable) >= 0 || this.zzon.containsKey(comparable);
    }
