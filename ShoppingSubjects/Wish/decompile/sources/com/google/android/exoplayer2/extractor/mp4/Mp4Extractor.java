    ...
    private void processMoovAtom(ContainerAtom containerAtom) throws ParserException {
        ...
        for (int i2 = 0; i2 < containerAtom2.containerChildren.size(); i2++) {
            ...
            if (containerAtom3.type == Atom.TYPE_trak) {
                Track parseTrak = AtomParsers.parseTrak(containerAtom3, containerAtom2.getLeafAtomOfType(Atom.TYPE_mvhd), -9223372036854775807L, null, (this.flags & 1) != 0, this.isQuickTime);
                if (parseTrak != null) {
                    ...
                    if (parseStbl.sampleCount != 0) {
                        ...
                        long max = Math.max(j, parseTrak.durationUs != -9223372036854775807L ? parseTrak.durationUs : parseStbl.durationUs);
                        if (parseTrak.type == 2) {
                            if (i == -1) {
                                i = arrayList.size();
                            }
